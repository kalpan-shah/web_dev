...
* **Collector Forwarding**: The OTel Collector receives logs on OTLP port `4317` and forwards them to Loki via the `otlphttp/loki` exporter configured in [`otel-config.yaml`](./otel-config.yaml) targeting `http://loki:3100/otlp`.
...
- [`LOKI_LOGS_TROUBLESHOOTING.md`](./LOKI_LOGS_TROUBLESHOOTING.md): 5-Level diagnostic helper manual for Loki & OTel logging issues.
